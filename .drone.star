...
def testPipelines(ctx):
  ...
  pipelines.append(settingsUITests(ctx, config['uiTests']['webBranch'], config['uiTests']['webCommit']))
  return pipelines
...
def settingsUITests(ctx, webBranch, webCommit, storage = 'owncloud', accounts_hash_difficulty = 4):
  return {
    'kind': 'pipeline',
    'type': 'docker',
